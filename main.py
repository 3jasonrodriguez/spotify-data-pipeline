...
def app():
    ...
    try:
        ...
        with get_postgres_conn() as conn:
            ...
            #Remove compare from visualization page
            user_list_options = [u for u in get_users(conn) if u != "compare"]
            #Available user selection
            user = st.sidebar.selectbox(
                "Select User",
                options=user_list_options,
                index=user_list_options.index("jason") if "jason" in user_list_options else 0
            )
            kpis(conn, user)
